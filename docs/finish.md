...
## 진행 현황 (최종 갱신 2026-09-16, 멀티 결함 11건 + 로그인 오픈 리다이렉트 수정 반영)
...
| 3 | 백업 cron + 복원 리허설 | ✅ 리허설 2026-09-14 / cron 2026-09-15 / 오프사이트 ➖ 구축 안 함(2026-09-15 결정) | 수동 백업(`/root/backup/song-20260914-2104.sql.gz`, 31 테이블) → 임시 컨테이너 복원(적재 1.2초, 6개 표 행수 일치) → 복원 DB로 앱 기동 UP 33초·`validate` 통과. cron 은 09-15 구축. 오프사이트는 개인 프로젝트 규모에서 감수하기로 결정(runbook §4 에 기록) |
| 4 | runbook 초안 `docs/runbook.md` | 🟡 §1·§4-1·§4-2·§5(명령만)·§6 검증 2026-09-14 / **§2·§3 검증 2026-09-15** | 상태 확인·수동 롤백(`start`로 직전 색 복귀)·SHA 재배포·DB 백업/복원 리허설/운영 복원·재부팅 점검·비밀번호 교체·만료 항목·증상표. 2026-09-15 리허설: §2 green→blue(`cf1701a`) 전환·원복 각 외부 200, blue 종료 `143`; §3 유휴 blue에 `9c12679` 45초 UP 후 `latest` 원복. 발견: root 셸 `cp -i` 별칭이 `.bak` 재작성 시 프롬프트 → `\cp`로 수정. §7 만료일 3개 기입 2026-09-15(도메인 2028-03-23 RDAP, Docker Hub 토큰 Never, VPS 2027-03-22 자동연장). 남은 🔲: 실제 재부팅 리허설 |
...
| 보안 | STOMP 구독 인가 + 폴링 GET(`/round`·`/chats`) 참가자 검사, 도달 불가 `WebSocketAuthInterceptor` CONNECT 블록 제거 | ✅ 2026-09-15 (운영 반영 ✅ 17:25, `5405aa5` 배포 · `/ws/websocket` 101 · 비로그인 `/round` 401 확인) | `06a6e4a`. 배경: 방 코드만으로 비참가자·비로그인이 토픽 구독·`/round`·`/chats` 수신 가능. 인터셉터는 `HTTP_SESSION` 키를 읽으나 채우는 곳 없음(핸드셰이크가 이미 Principal 전파 — dev 프로브로 실측). `/status` 는 참가 전 미리보기(`multi-join.js`)가 써서 제외. 검증: 신규 테스트 17건, `./mvnw clean test` 335건 통과, dev 실방 STOMP 프로브로 비로그인 SUBSCRIBE ERROR·`/round` 401, 2브라우저+시크릿+C계정 수동 시나리오 9건 통과(사용자). 상세 `docs/ws-subscription-authorization.md` |
| 버그 | 방 생성 시 비공개 플래그·게임 모드가 저장되지 않음 + 죽은 참가 엔드포인트 | ✅ 2026-09-15 (운영 반영 ✅ 17:25, `5405aa5` 배포 · `/ws/websocket` 101 · 비로그인 `/round` 401 확인) | `81c01ed`. 배경: `multi-create.js` 가 `isPrivate` 와 중첩 `settings:{gameMode,…}` 로 보내는데 서버는 본문 전체를 `GameSettings`(`privateRoom`, 최상위 필드)로 바인딩 → Jackson 이 미지 키를 조용히 버려 **모든 방이 공개·RANDOM** 으로 저장(로컬 DB 실측 `is_private=0`, `gameMode:"RANDOM"`). 비공개 방이 로비 목록에 뜨고 입장됨, 장르/아티스트/연도 고정이 곡 선택에 반영된 적 없음. `ded5428`(01-09) 부터. 결정(사용자, 옵션 A): 비공개 = 목록 숨김 + 코드 입장, 비밀번호 없음(생성 라벨 그대로). `/join/{code}` 비밀번호 검사(저장된 비밀번호가 없어 항상 거부)·참가 페이지 비밀번호 입력 제거, 검사 없는 `/room/{code}/join`(호출 0건) 삭제. 검증: `MultiGameControllerRoomLifecycleTest` 9건(생성 본문 저장·목록 제외·코드 참가·404·JS↔DTO 키 계약), `./mvnw test` 344건 통과. `GameRoom.password` 컬럼은 미사용으로 남음(스키마 변경이라 보류). **후속 `cb1f951`** (사용자 결정): 비공개 방도 로비 목록에 🔒 표시로 보이되 **코드는 DOM·JSON 어디에도 내려보내지 않음**, 카드 버튼은 "코드를 입력하라" 안내만 하고 return. 목록/검색 쿼리에서 `isPrivate=false` 제거. 테스트 10건, 전체 345건 통과 |
| 버그 | 대기실 `ROOM_UPDATE` push 수신 시 로비로 튕김 | ✅ 2026-09-15 (운영 반영 ✅ 17:25, `5405aa5` 배포 · `/ws/websocket` 101 · 비로그인 `/round` 401 확인) | `a155717`. 위 작업 수동 검증 중 발견: `multi-waiting.js handleRoomUpdate` 가 `!payload.success` 로 "방 종료" 판단하는데 push payload(`buildRoomStatus`)에는 `success` 키가 없어 B 참가·준비 토글마다 구독 중인 전원이 로비로 이동. `f3d36cc`(04-06) 부터 잠복 — 운영은 09-14 까지 WebSocket 미성립이라 폴링만 돌아 드러나지 않음. `=== false` 로 1줄 수정, A/B 재확인 통과 |
| 버그 | 멀티 소스 전수 분석(2026-09-16, 어제 2건과 같은 계열) → 12항목 중 11건 수정 | ✅ 2026-09-16 코드·테스트 (운영 반영 🔲 미푸시 — **2026-09-21 정정: 실제로는 09-16 07:44 배포(run `5d77fef`)에 포함됨. ③ 은 게임 시작 8초 뒤 전원 이탈 결함 → `docs/verification/records/2026-09-21_multi-unload-token.md`**) | `93963e5`~`21ffe40` 7커밋 + docs `8a965a6` + `1ce90b4`~`620ec28` 4커밋, 항목별 실패 테스트 선행. ① 정원이 LEFT 행까지 세어 한 번 나간 자리를 못 채움(`getCurrentPlayerCount`·`SIZE(r.participants)`) → 활성 참가자 기준. ② `leaveRoom` 이 PLAYING 을 무시해 "게임 종료" 를 눌러도 PLAYING 으로 남아 다른 방 생성 불가, 로비 배너는 JOINED 만 조회, 방장 사라진 PLAYING 방은 배치 대상 아님 → 게임 중 실제 나가기+위임, 배너 PLAYING 포함, 2시간 방치 PLAYING 방 종료. ③ 탭 닫기 `sendBeacon(/leave)` 가 CSRF 403 으로 도달한 적 없음(fetch 래퍼만 토큰 첨부, 예외는 `/ws/**` 뿐) → `/unload` CSRF 예외 + `RoomUnloadService` 8초 유예 후 적용, 페이지 GET 시 취소(F5·게임 내 이동 보호). ④ CHAT push 가 `id`·`memberId`·`isHost` 없이 `CORRECT` 타입 → 폴링 항목과 동일 `toChatInfo`. ⑤ 시스템 메시지 미push → 저장 직후 push. ⑥ `/chat`·`/skip-vote`·`/round-ready` 가 LEFT 참가자 허용(강퇴자가 정답으로 득점) → 활성 참가자만. ⑦ `nextRound`·`skipCurrentSong` 상태 검사 없어 FINISHED 방 재호출 시 `finishGame` 재실행(LP 이중) → PLAYING 만 허용. ⑧ 결과 페이지가 `results[0]` 을 무조건 읽어 전원 0점 LEFT 방의 결과 URL 재방문 시 500 → 빈 결과면 로비로. ⑨ `ws-client.js` 가 CONNECT 성공마다 시도 횟수를 0 으로 초기화 → 구독 거부 사용자는 폴백 없이 1초마다 무한 재접속(분석 때 지목한 ERROR+close 이중 재연결은 node 하네스에서 재현 안 됨 — 첫 콜백이 `connected` 를 내려 두 번째는 건너뜀) → 연결 5초 유지 시에만 초기화, 폴백 1회 보장, 대기실·플레이 폴링 인터벌 중복 방지. ⑩ 방 생성 서버 검증 없음(빈 이름·50자 초과 DB 오류 500, `maxPlayers:null` NPE 500) → `validateRoomSettings`(이름 2~30·인원 2~10·라운드 1~20, 실패는 `success:false`). ⑪ 참가 링크 로그인 리다이렉트 code 유실 — **결함 아님**(쿼리 값 안의 `?` 는 구분자가 아님, 테스트로 확인 후 폐기). ⑫ `/round-ready` 는 `PREPARING` 을 요구하나 어디서도 설정되지 않고 호출도 없음 → 엔드포인트·서비스 메서드 3개 제거(`round_ready` 컬럼·enum 은 스키마라 유지). ⑬ 검토 중 발견: 로그인 `redirect` 파라미터가 검증 없이 `window.location.href` 로 → 오픈 리다이렉트. `AuthController.sanitizeRedirect` 로 `/` 시작 내부 경로만 허용(`//host`·스킴·백슬래시·제어문자 거부). 검증: 신규 테스트 45건(`GameRoomCapacityTest` 3·`GameRoomLeaveDuringPlayTest` 7·`MultiGameControllerUnloadTest` 5·`MultiGameControllerChatPushTest` 4·`MultiGameServiceLeftParticipantTest` 2·`MultiGameServiceFinishedRoomGuardTest` 2·`MultiGameControllerResultPageTest` 1·`MultiGameControllerCreateValidationTest` 10·`MultiGameControllerRoomLifecycleTest` +1·`AuthControllerLoginRedirectTest` 9), `./mvnw test` 389건 통과. ⑨ 는 JS 라 Maven 밖 — Jest 를 제거한 저장소 결정(§1-3)에 따라 스크래치 node 하네스로 수정 전 실패·후 통과만 확인, 저장소에는 미포함. 운영 반영 후 수동 확인 예정: 대기실 F5 자리 유지 / 탭 닫고 8초 뒤 자리 해제·방장 위임 / 플레이 중 게임 종료 후 방 만들기 / WS 상태에서 정답 강조·시스템 메시지 |
