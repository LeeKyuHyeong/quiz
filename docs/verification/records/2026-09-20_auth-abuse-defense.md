...
- 유형: 버그 (보안)
- 우선순위: P0 (인증)
- 판정: 수용 가능 — 자동 검증·로컬 MariaDB·화면 확인·운영 배포 Smoke ✅ (2026-09-21). 남은 것은 버튼 대기 시간 수정의 화면 확인 1건(§8)
...
| A2-12 | 정상 | 로그인 화면에 잠금 안내 문구가 보이고, 올바른 비밀번호로도 같은 안내 | ✅ | 개발자가 dev(18082)에서 확인 2026-09-20 — 6회째 잠금 안내, 올바른 비밀번호도 거부. (429 문구는 dev 접속 주소가 화이트리스트라 화면으로는 못 봄 — 자동 테스트 A2-1) |
...
| A3-4 | 정상 | 가입·재설정 화면에 재발송 안내 문구가 보인다 / 비밀번호 재설정 뒤 잠금 해제(화면) | ✅ | 개발자가 운영에서 확인 2026-09-21 — 안내 문구("…29초 뒤에 다시 시도해주세요") 표시, 재설정 뒤 바로 로그인. 이때 버튼 대기 시간 불일치 발견(§8) |
...
| 사용자 시나리오 | §6-3 1번 — 개발자가 dev(18082)에서 수행 (2026-09-20). 2·3번은 메일 발송이 필요해 운영으로 | 1번 기대대로 | ✅ |
| 운영 DB | 수동 백업(284KB, 권한 600) → ALTER (2026-09-20 23:5x, 개발자 SSH) | 컬럼 2개 생성, 회원 46명 `login_fail_count` 합 0 | ✅ |
| 운영 반영·Smoke | 2026-09-21 `f52650d` 배포 (Actions run 35518153306: 테스트 → 유휴 색 헬스체크 → upstream 전환). 외부(브라우저 UA·GET): `/`·`/auth/login`·`/game/solo/guess`·`/board`·`/stats` 200, `/mypage` 302, `/actuator/health` 403. **`X-Forwarded-For` 를 요청마다 바꿔 `/auth/check-email` 22회 → 200×20, 429×2.** 개발자: 로그인 이력 IP = 본인 공인 IP, 중복 로그인 시 먼저 있던 창 로그아웃(O-010) | 통과 | ✅ |
...
- **재발송 버튼 대기 시간 불일치**(운영 확인 중 개발자가 발견, 2026-09-21): 화면의 재발송 버튼은 30초 뒤 다시 눌리는데 서버는 60초까지 거부 → 눌러도 소용없는 버튼이 30초간 보였다. 호출처 조사에서 JS(`auth-register.js`·`auth-password-reset.js` 의 `startResendCooldown(30)`)를 보지 않은 것이 원인 → 60 으로 맞춤. JS 는 Maven 밖이라 자동 테스트 없음 — 배포 후 화면 확인 🙋(버튼이 60초 막히고, 풀린 뒤 누르면 발송된다. 브라우저 캐시 때문에 Ctrl+F5 후 확인) → **확인 완료(개발자 운영, 2026-09-21)**: 운영 JS 두 파일 모두 `startResendCooldown(60)`·`Cache-Control: no-store`(curl) / 코드 받기 → `재발송 (60s)` 부터 감소·메일 도착 → 60초 뒤 버튼 활성 → 재클릭 시 에러 없이 두 번째 메일 도착. 안내 문구 "(5분 내 유효)" 는 `CODE_TTL_MINUTES = 5` 와 일치해 유지. O-017 닫힘
