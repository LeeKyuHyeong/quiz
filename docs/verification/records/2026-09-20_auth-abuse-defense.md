...
- **재발송 버튼 대기 시간 불일치**(운영 확인 중 개발자가 발견, 2026-09-21): 화면의 재발송 버튼은 30초 뒤 다시 눌리는데 서버는 60초까지 거부 → 눌러도 소용없는 버튼이 30초간 보였다. 호출처 조사에서 JS(`auth-register.js`·`auth-password-reset.js` 의 `startResendCooldown(30)`)를 보지 않은 것이 원인 → 60 으로 맞춤. JS 는 Maven 밖이라 자동 테스트 없음 — 배포 후 화면 확인 🙋(버튼이 60초 막히고, 풀린 뒤 누르면 발송된다. 브라우저 캐시 때문에 Ctrl+F5 후 확인)
