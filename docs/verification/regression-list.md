...
| R-031 | Multi | 서버 기동 뒤 `game.multi.startup-sweep-delay-ms` 가 지나면 진행 중인 방(WAITING·PLAYING)에서 방 토픽 연결이 없는 참가자는 끊김 유예 뒤 나간다(서버가 내려간 사이 창을 닫은 사람). 구독 중·폴링 중인 화면은 남고, 종료된 방은 대상이 아니며, 지연이 음수면 돌지 않는다 | `RoomPresenceStartupSweepTest`(4) + `RoomPresenceStartupSweepReadyTest`(1, 기동 이벤트 경로) + 운영: 배포 뒤 `docker logs` 의 `Presence startup sweep` | records/2026-09-22_startup-sweep | 2026-09-22 |
| R-033 | Auth/Config | 세션 저장소 프로파일이 서로 새지 않는다 — `test`·`session-jdbc`(= prod)에는 `RedisConnectionFactory` 가 없고 JDBC 저장소가 잡히며, `session-redis` 만 indexed Redis 저장소 + WS 가드 + Redis 헬스. Redis 의존성이 클래스패스에 있어도 prod 기동·헬스에 영향 없음 (의존성만 추가하면 prod 기동 실패 — 빨강으로 확인) | `SessionStoreProfileTest` 3중첩 4건 · `SessionLifecycleRedisContractTest` 9건(Docker 필요) | records/2026-09-22_session-redis-profile | 2026-09-22 |
| R-032 | Security/Test | 로그인 요청 제한 테스트는 벽시계와 무관하다 — 제한기의 시계를 주입해 20회 200 → 21번째 429 → 2초 뒤 429 → 4초 뒤 1회 200(greedy 3초당 1개)을 고정. 느린 PC 에서 로그인 20회가 3초를 넘겨도 결과가 같다 | `LoginAttemptLimitTest.loginRequests_areRateLimitedPerIp` (`ManualClock` `@Primary` 빈) | records/2026-09-22_rate-limit-clock | 2026-09-22 |
