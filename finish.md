...
| 버그 | 방 생성 시 비공개 플래그·게임 모드가 저장되지 않음 + 죽은 참가 엔드포인트 | ✅ 2026-09-15 (운영 반영 🔲) | `81c01ed`. 배경: `multi-create.js` 가 `isPrivate` 와 중첩 `settings:{gameMode,…}` 로 보내는데 서버는 본문 전체를 `GameSettings`(`privateRoom`, 최상위 필드)로 바인딩 → Jackson 이 미지 키를 조용히 버려 **모든 방이 공개·RANDOM** 으로 저장(로컬 DB 실측 `is_private=0`, `gameMode:"RANDOM"`). 비공개 방이 로비 목록에 뜨고 입장됨, 장르/아티스트/연도 고정이 곡 선택에 반영된 적 없음. `ded5428`(01-09) 부터. 결정(사용자, 옵션 A): 비공개 = 목록 숨김 + 코드 입장, 비밀번호 없음(생성 라벨 그대로). `/join/{code}` 비밀번호 검사(저장된 비밀번호가 없어 항상 거부)·참가 페이지 비밀번호 입력 제거, 검사 없는 `/room/{code}/join`(호출 0건) 삭제. 검증: `MultiGameControllerRoomLifecycleTest` 9건(생성 본문 저장·목록 제외·코드 참가·404·JS↔DTO 키 계약), `./mvnw test` 344건 통과. `GameRoom.password` 컬럼은 미사용으로 남음(스키마 변경이라 보류) |
