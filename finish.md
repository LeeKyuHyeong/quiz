...
| 4 | runbook 초안 `docs/runbook.md` | 🟡 §1·§4-1·§4-2·§5(명령만)·§6 검증 2026-09-14 / **§2·§3 검증 2026-09-15** | 상태 확인·수동 롤백(`start`로 직전 색 복귀)·SHA 재배포·DB 백업/복원 리허설/운영 복원·재부팅 점검·비밀번호 교체·만료 항목·증상표. 2026-09-15 리허설: §2 green→blue(`cf1701a`) 전환·원복 각 외부 200, blue 종료 `143`; §3 유휴 blue에 `9c12679` 45초 UP 후 `latest` 원복. 발견: root 셸 `cp -i` 별칭이 `.bak` 재작성 시 프롬프트 → `\cp`로 수정. 남은 🔲: 실제 재부팅 리허설, §7 만료일(도메인·Docker Hub 토큰·VPS) |
