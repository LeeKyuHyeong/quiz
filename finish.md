...
## 진행 현황 (최종 갱신 2026-09-16, 멀티 결함 7건 수정 반영)
...
| 버그 | 멀티 소스 전수 분석(2026-09-16, 어제 2건과 같은 계열) → 7건 수정 | ✅ 2026-09-16 코드·테스트 (운영 반영 🔲 미푸시) | `93963e5`~`21ffe40` 7커밋, 항목별 실패 테스트 선행. ① 정원이 LEFT 행까지 세어 한 번 나간 자리를 못 채움(`getCurrentPlayerCount`·`SIZE(r.participants)`) → 활성 참가자 기준. ② `leaveRoom` 이 PLAYING 을 무시해 "게임 종료" 를 눌러도 PLAYING 으로 남아 다른 방 생성 불가, 로비 배너는 JOINED 만 조회, 방장 사라진 PLAYING 방은 배치 대상 아님 → 게임 중 실제 나가기+위임, 배너 PLAYING 포함, 2시간 방치 PLAYING 방 종료. ③ 탭 닫기 `sendBeacon(/leave)` 가 CSRF 403 으로 도달한 적 없음(fetch 래퍼만 토큰 첨부, 예외는 `/ws/**` 뿐) → `/unload` CSRF 예외 + `RoomUnloadService` 8초 유예 후 적용, 페이지 GET 시 취소(F5·게임 내 이동 보호). ④ CHAT push 가 `id`·`memberId`·`isHost` 없이 `CORRECT` 타입 → 폴링 항목과 동일 `toChatInfo`. ⑤ 시스템 메시지 미push → 저장 직후 push. ⑥ `/chat`·`/skip-vote`·`/round-ready` 가 LEFT 참가자 허용(강퇴자가 정답으로 득점) → 활성 참가자만. ⑦ `nextRound`·`skipCurrentSong` 상태 검사 없어 FINISHED 방 재호출 시 `finishGame` 재실행(LP 이중) → PLAYING 만 허용. 검증: 신규 테스트 24건(`GameRoomCapacityTest` 3·`GameRoomLeaveDuringPlayTest` 7·`MultiGameControllerUnloadTest` 5·`MultiGameControllerChatPushTest` 4·`MultiGameServiceLeftParticipantTest` 3·`MultiGameServiceFinishedRoomGuardTest` 2), `./mvnw test` 369건 통과. 운영 반영 후 수동 확인 예정: 대기실 F5 자리 유지 / 탭 닫고 8초 뒤 자리 해제·방장 위임 / 플레이 중 게임 종료 후 방 만들기 / WS 상태에서 정답 강조·시스템 메시지 |
